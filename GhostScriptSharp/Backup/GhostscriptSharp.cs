...
namespace GhostscriptSharp
{
	...
	public class GhostscriptWrapper
	{
		...
		public static void GeneratePageThumb(string inputPath, string outputPath, int page, int dpix, int dpiy, int width, int height)
		{
            int pagecount = getpagecount(inputPath);
            //var result = MessageBox.Show(Uri.UnescapeDataString(inputPath), "info");
            //result = MessageBox.Show(Convert.ToString(pagecount), "Error");
            for (int i = 0 ; i <= pagecount ; i++)
            {
                //result = MessageBox.Show(Uri.UnescapeDataString(inputPath),"info");
                //result = MessageBox.Show(Convert.ToString(pagecount), "Error");
                GeneratePageThumbs(inputPath, outputPath, 1, i, dpix, dpiy, width, height);
            }
		}
		...
		public static void GeneratePageThumbs(string inputPath, string outputPath, int firstPage, int lastPage, int dpix, int dpiy, int width, int height)
		{
            if (IntPtr.Size == 4)
        ...
        public static int getpagecount(string vfileName)
        {
            //need to check file exists try statement
            string PgCount = string.Empty;
            System.IO.FileInfo fextension = new FileInfo(Uri.UnescapeDataString(vfileName));
            ...
            try
            {
                return Convert.ToInt32(PgCount);
            }
